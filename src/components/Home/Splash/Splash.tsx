...
export const Splash = () => {
  useEffect(() => {
    // Disable scrolling while the animation is running. Lenis intercepts
    // wheel/touch events itself, so body overflow alone won't stop it.
    ...
    const timeout = setTimeout(unlockScroll, 3700); // Match the total animation duration (3.7 seconds)

    // If the user navigates away before the animation finishes, the timeout
    // never fires — unlock here too, or scroll stays dead on every page.
    return () => {
  ...
  return (
    <>
      <motion.div
        className="absolute bg-black light:bg-[#f7f5f2] h-screen z-20 inset-0"
        initial={{ opacity: 1 }}
      ...
      >
        <div className="container md:scale-125 lg:scale-150">
          <motion.svg
